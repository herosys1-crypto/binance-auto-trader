...
## 🛡 Phase 4 — 실시간 변경 위험 분석 + 사장님 결정 (2026-06-08 추가)

### 사장님 요구 (2026-06-08):
> "실행된 전략 인스턴스에서 변경할 수 있게 만들어줘
>  익절 TP3 단계 실행 후 청산하는 옵션에 문제없게 기획해줘"

### 위험 시나리오 (= 식별):

#### Risk: Retrace 줄임 시 즉시 청산
```
사장님 옵션 -20% 운영 중
peak +30% / 현재 +24% (= 6% 회귀, -20% 미달 = 청산 X)
   ↓
사장님이 -20 → -5 변경 (= 보수 의도 또는 실수)
   ↓
즉시 risk cycle → peak - 5% = +25% > +24% → TRAILING_TP 즉시 발동!
   ↓
전량 청산 = 사장님 의도 X 시 = 큰 자본 침해
```

### 시점별 영향 매트릭스:

| Status | Retrace 변경 영향 |
|---|---|
| WAITING / ENTERED (TP3 미발동) | armed X = 영향 0 ✅ |
| TP1~TP2 발동 (TP3 전) | armed X = 영향 0 ✅ |
| TP3+ 발동 + ARMED | ⚠️ 즉시 적용 (안전: 늘림 / 위험: 줄임) |
| Crisis | TP3 발동 시 위와 동일 |
...
- **Audit log (RiskEvent TRAILING_RETRACE_UPDATED) 영구 기록** (= 이미 구현)
- **사장님 자율 책임** (= 사장님 사상 우선)

...
| **손절 룰** | 총 자본 -50% | **첫 TP (+5%) 발동 후 PnL -1% → 전량 즉시 ⬇️** |

### 2-3-1. ⚠️ 중요 — 크라이시스 모드 단계별 활성화

크라이시스 모드는 **두 단계** 로 발동됩니다:

```
[Stage 1] 크라이시스 모드 진입
  조건: (5+ 단계 진입 OR 마지막 단계 진입) + 최대 손실 ≥ 30%
  효과: TP1 임계 +5% 로 전환 (다른 룰은 그대로)
  손절: 여전히 정상 모드의 총 자본 -50% 적용

[Stage 2] 첫 TP (+5%) 발동 후 — 보호 활성화
  발동 시점: TP1 (+5%) 청산 완료 (status=CRISIS_TP1_DONE)
  효과:
    - 트레일링 -5% 활성화: 피크 PnL 추적 → 피크에서 -5% 회귀 시 전량 청산
    - 빠른 손절 -1% 활성화: PnL 가 -1% 이하로 떨어지면 전량 즉시 손절
  의미: "한 번 차익 봤으니, 다시 빠지면 미련 없이 탈출"
```

따라서 **TP1 (+5%) 이 발동되기 전까지** 는 -1% 손절 / -5% 트레일링이 작동하지 않음. 시세가 -30% 까지 빠진 상태에서 +5% 도달 전에 또 더 빠지더라도 정상 모드의 -50% 손절만 작동.
...
진입 깊고(5+ 단계) 손실 깊었던(-30%) 케이스
    │
    └─ 🚨 크라이시스 모드 [Stage 1] 진입
            │  (TP1 임계만 +5% 로 변경, 손절은 여전히 -50%)
            │
            ├─ 시세 회복 안 됨 → -50% 손절 (정상 모드 룰 유지)
            │
            └─ 시세 회복 → +5% 도달
                  │
                  └─ ⚡ TP1 발동 (25% 청산)
                        │
                        └─ 🛡 크라이시스 모드 [Stage 2] 보호 활성화
                              (트레일링 -5% + 빠른 손절 -1%)
                              │
                              ├─ 시세 더 오름 → +10% / +15% / ... → 기존 TP2~5 룰
                              │     │
                              │     └─ 도중에 피크 -5% 회귀 → 남은 전량 청산 🛡
                              │
                              └─ 시세 하락 → PnL -1% 도달 → 남은 전량 손절 🚨
```
...
eval_crisis(strategy, pnl_pct):
    # ───── Stage 1 (TP1 미발동) ─────
    # +5% 도달 전: 손절은 여전히 정상 모드의 -50% 룰만 적용
    if not strategy.crisis_first_tp_done_at:
        # +5% 도달 시 첫 TP
        if pnl_pct >= 5:
            return ("CRISIS_TP1", 25%)  # 25% 청산
        # 그 외엔 정상 모드 손절 -50% 그대로 사용 (eval_normal 의 SL 검사로 위임)
        return eval_normal_sl_only(strategy, pnl_pct)

    # ───── Stage 2 (TP1 발동 후 — 보호 활성화) ─────
    # 피크 PnL% 추적 (트레일링용)
    strategy.peak_pnl_pct_after_first_tp = max(
        strategy.peak_pnl_pct_after_first_tp or pnl_pct, pnl_pct
    )
...
### 시나리오 A: 정상 회복 (트레일링 보호 발동)
```
1. 전략 시작 → 1~5단계 모두 진입
2. 시세 급등(SHORT 기준 손실) → PnL -32% 도달 → max_loss = -32% 기록
3. 🚨 크라이시스 모드 [Stage 1] 진입 → Telegram 알림
   (이 시점에 -50% 손절은 여전히 작동, -1% 손절은 아직 비활성)
4. 시세 회복 → PnL +5% 도달 → CRISIS_TP1 (25% 청산) → Telegram
5. 🛡 크라이시스 [Stage 2] 보호 활성화 (-5% 트레일링 + -1% 손절 ON)
6. 시세 더 오름 → +12% 도달 → 기존 TP2 (+15%) 까지 대기 (피크 +12% 갱신)
7. 시세 약간 하락 → +7% 도달 → 피크(+12%) - 5% = 7% 도달 → CRISIS_TRAIL_FULL → 남은 전량 청산
```

### 시나리오 B: 회복 후 재하락 (-1% 빠른 손절)
```
1~5. (시나리오 A 와 동일하게 +5% 도달 → CRISIS_TP1)
6. 보호 활성화 후 시세 다시 급락
7. PnL +3% → +1% → 0% → -1% 도달 → CRISIS_HARD_SL → 남은 전량 손절
```

### 시나리오 C: TP1 발동 전 추가 손실 (정상 모드 -50% 손절)
```
1~3. (크라이시스 모드 진입 후 +5% 도달 못함)
4. 시세가 -32% 에서 더 빠짐 → -45% → -50% 도달
5. 정상 모드 -50% 손절 발동 (CRISIS_HARD_SL 아님 — 아직 TP1 발동 안 했으니)
   재진입 대기 (manual_ready)
```

이 3가지 시나리오 모두 단위 테스트로 작성 권장.

...
class TestTrailingFiresWhenAboveTPThreshold:
    ...
    def test_98_LABUSDT_scenario_trailing_fires_at_peak_retrace(
        ...
        Peak 30%, 현재 19.76% (drop 10.24%) → 트레일링 발동 기대 (v4=v2 revert, 2026-05-12 저녁)."""
        tpl = make_template(
        ...
        # leverage=2 가정: raw -10.13% × 2 = leveraged ROI ~+20.25%
        ...
        assert result == "TRAILING_TP", (
            ...
            f"현재 status TP3_DONE_PARTIAL — 활성 trailing armed status (v4). "
            f"실제 결과: {result}"
        )
    ...
    def test_trailing_fires_for_tp3_plus_done_partials(
        ...
        """TP3+ partial 에서만 trailing 발동 (v4 = v2 revert, 2026-05-12 저녁).
    ...
    def test_trailing_NOT_armed_for_tp1_tp2_done_partials(
        ...
        """TP1/TP2 발동만으로는 trailing 활성 X (v4 = v2 revert, 2026-05-12 저녁).
        ...
        # 대신 다음 미발동 TP (TP1→TP2, TP2→TP3) 반환.
        assert result != "TRAILING_TP", f"{done_status}: trailing 발동되면 안 됨 (정책 v4)"

...
3가지 시나리오:
  A. 정상 회복 — -32% → +5% TP1 → +12% 피크 → +7% 회귀 → 트레일링 청산
  B. 재하락 — -32% → +5% TP1 → -1% → 빠른 손절
  C. TP1 발동 전 추가 손실 — -32% → -50% → 정상 모드 손절
"""
...
def test_crisis_constants() -> None:
    """크라이시스 모드 임계 상수가 운영자 의도와 일치하는지."""
    assert CRISIS_MIN_STAGE == 5
    assert CRISIS_MAX_LOSS_THRESHOLD == Decimal("-30")
...
# ──────────── 시나리오 E: 5단계 미만은 트리거 안 됨 ────────────
def test_no_crisis_under_5_stages() -> None:
    """4단계 진입 + -35% 손실이어도 크라이시스 모드 진입 안 함 (현재 룰)."""
    # 이건 _should_trigger_crisis_mode 의 책임 — 여기선 룰 자체 검증만
    # 실제 통합 테스트는 RiskService 인스턴스 + DB mock 으로 별도 작성 필요
    assert CRISIS_MIN_STAGE == 5  # 현재 룰: 5단계 미만은 트리거 안 됨

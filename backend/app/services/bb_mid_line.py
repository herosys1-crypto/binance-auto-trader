...
## ⚠️ 익절 구조는 **일부러** 사다리다 — 「+5% 전량」으로 바꾸지 말 것

2026-09-02 감사에서 「백테스트는 +5% 전량 청산 가정인데 실제는 25%만 청산한다.
기대값이 뒤집힌다」는 지적이 나왔다. **사실관계는 맞지만 결론은 틀렸다.**
같은 표본에 실제 익절 모형(TP1~4 각 25% + 트레일링 peak-3)을 넣어 다시 재니:

    규칙                  +5% 전량(가정)     실제 사다리
    중단 저항 SHORT       731건 건당 +0.821   731건 건당 **+1.132**   (둘 다 과적합 통과)
    중단 하락돌파 SHORT   794건 건당 +0.718   794건 건당 **+1.169**

크게 뻗는 거래에서 사다리가 더 벌어서, 부분 익절 쪽이 **40~60% 낫다**.
게다가 tp1_qty_ratio=100 으로 바꿔도 tp_sl_orchestrator 의 v127 안전망이
(level_n < 20 이고 ratio>=100 이면) 조용히 25 로 되돌린다 — 바꾸려면 그 안전망까지
건드려야 하는데, 실측이 바꾸지 말라고 한다.

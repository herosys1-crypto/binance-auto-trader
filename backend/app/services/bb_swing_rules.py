...
규칙 (전부 **완성된 15분봉**, 흐름은 **완성된 4시간봉**):
  ...
  분할    = 볼밴 분할(split_entry)과 같은 실행 경로 — 1차 시장가 · 2·3차 실체결가 재앵커 · 평단 ROI 손절 전량 ·
            TP1 부터 25%씩 · 트레일링. 손절가는 가격상 2·3차 트리거보다 뒤에 있다(check_no_dead_stage 검산) = 사장님
            「모두 진입하고도 -10% 손실이면 청산」. ⚠️ 단 2·3차는 볼밴 분할 추가 게이트(정점-주춤·조정 신호)를 통과해야 들어가므로
            1차만 든 채 손절될 수 있다 (반박 검증 2026-09-15 H2).
...
SETTINGS: dict[str, tuple[str, str, str]] = {
    ...
    "bb_swing_capitals": ("10,100,200", "1·2·3차 증거금 USDT", "사장님 2026-09-15 「10 100 200 이렇게 진입」"),

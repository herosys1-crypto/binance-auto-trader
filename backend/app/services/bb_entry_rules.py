...
## 🚨 「봉수를 낮춰 큰 정점을 잡자」 — **네 번 시도해서 네 번 다 나빠졌다**

2026-09-02 USELESSUSDT 사례(0.0586 → 0.1260 = +115%)에서 우리 규칙이 정점을 놓쳤다.
상단 밖이 **2봉**뿐이었고 우리는 4봉을 요구했기 때문이다. 완화를 네 가지로 시도했다:

    ① 급등이 격렬하면 밴드가 벌어져 봉수가 안 쌓인다  → 반증
       4,109개 구간에서 4봉 이상 비율이 급등 강도와 무관하게 11~15% 로 평평
    ② 「직전 6시간 급등률」을 조건으로 쓴다            → 반증
       건당 +1.681 → **-0.854** (6h>=6%) / **-2.500** (6h>=10%)
       하락폭은 커지지만 변동성 때문에 손절에 먼저 걸린다
    ③ 「밴드 복귀」(밖에 있다가 안으로 돌아옴)를 추가  → 반증
       건당 +1.684 → +0.773 (합계는 +11% 늘지만 건당이 절반)
    ④ 봉수를 2로 낮춘다                                → **상한이 있으면 반증**
       무제한   2봉 +434 / 4봉 +296   ← 2봉 승
       상한 10  2봉 +152 / 4봉 **+196** ← 4봉 승
       상한 5   2봉  +50 / 4봉  **+89**
       상한 3   3봉 **-18** (낮은 품질 신호가 슬롯을 먼저 채운다)

**슬롯이 병목이면 「총 수익」이 아니라 「슬롯당 수익」으로 판단해야 한다.**
정점을 놓치는 것은 결함이 아니라 규칙의 대가다 — 상단 밖 구간의 **87%를 의도적으로 버리고**
건당 수익을 2배로 만든다.

전체 기록: `docs/spec/CASE_USELESSUSDT_2026-09-02.md`

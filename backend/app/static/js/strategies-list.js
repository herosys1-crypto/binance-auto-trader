...
async function refreshStrategies() {
  try {
    ...
    tbody.innerHTML = sorted.map(s => {
      ...
      // = 진입 단계 절대 보존 + 미진입 단계 신 trigger_price (= 현재가 기준)
      const addStagesBtn = canTriggerNext
        ? `<button onclick="event.stopPropagation(); openAddUntriggeredStagesModal(${s.id}, '${s.symbol}', '${s.side}', ${s.current_stage || 0})" class="btn-ghost btn text-xs" style="${btnStyle}" title="미진입 단계 추가/수정 (4~10단계) — 진입 단계 절대 보존 + 신 단계 trigger = 현재가 × 1.10^N">➕</button>`
        : '';
      ...
      if (s.is_archived) {
        ...
        stopBtn = `<div class="flex flex-wrap gap-1" style="max-width:220px">

...
// v147d: 15분봉 **17.5~27.5%** 급등만 추격 LONG / 급락은 진입 비권장!
//   하위 밴드별로 TP/SL 이 다릅니다 — 17.5~22.5% = +5/-5(짧게), 22.5~27.5% = +15/-7(길게).
//   화면에 뜨는 TP/SL·기대값은 전부 백엔드 실측값을 그대로 받아 씁니다 (JS 하드코딩 금지!).
  ...
  function renderLivePumpDump(data) {
    ...
    const html = alerts.map((a, idx) => {
      ...
      const cfg = canEnter ? {
        ...
        force_sl_roi_override: 25,            // Fix 362 (2026-09-08 사장님): 새 전략 기본 -25% (옛 v166 = 5)

"""⛔ [폐지] 자동 증거금 추가 워커 — Fix 340 (2026-09-04 사장님 지시).

🚨 **이 워커는 더 이상 실행되지 않는다.** 스케줄러 등록이 제거됐고,
   `run_auto_add_margin()` 은 진입 즉시 반환한다.

사장님: "증거금 주입은 필요없는 기능이야. 지금 포지션 진입후 손실이면
        10usdt 남기고 부분손절입니다. 그런데 왜 증거금 주입이 필요한가
        이기능은 삭제해줘"

부분손절 사양(Fix 304~326)과 정면 충돌한다 — 손실 포지션에 300 을 더 넣어
버티는 것은 「10 USDT 남기고 잘라내고 다음 단계」와 반대다.
수동 증거금 추가(`POST /strategies/{id}/add-margin`)는 그대로 살아 있다.
...
def run_auto_add_margin() -> dict:
    # ⛔ Fix 340: 폐지된 기능. 어떤 경로로 불려도 아무것도 하지 않는다.
    #   (스케줄러 등록은 제거됐지만, 남은 호출처가 있어도 발주되지 않게 막는다.)
    logger.info("[Fix340] auto_add_margin 은 폐지된 기능입니다 — 아무것도 하지 않습니다 "
                "(사장님 2026-09-04: 부분손절 사양과 충돌). 수동 증거금 추가는 그대로 동작합니다.")
    return
